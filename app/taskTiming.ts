...
export function formatTaskWorkDuration(milliseconds: number, language: "tr" | "en" = "tr"): string {
  const safeMilliseconds = Math.max(0, milliseconds);
  const minute = 60_000;
  const hour = 60 * minute;
  const day = 24 * hour;
  if (safeMilliseconds < hour) {
    return `${Math.max(1, Math.ceil(safeMilliseconds / minute))} ${language === "tr" ? "dk" : "min"}`;
  }
  if (safeMilliseconds < day) {
    const hours = Math.floor(safeMilliseconds / hour);
    const minutes = Math.floor((safeMilliseconds % hour) / minute);
    return minutes
      ? `${hours} ${language === "tr" ? "sa" : "hr"} ${minutes} ${language === "tr" ? "dk" : "min"}`
      : `${hours} ${language === "tr" ? "sa" : "hr"}`;
  }
  const days = Math.floor(safeMilliseconds / day);
  const hours = Math.floor((safeMilliseconds % day) / hour);
  return hours
    ? `${days} ${language === "tr" ? "gün" : "d"} ${hours} ${language === "tr" ? "sa" : "hr"}`
    : `${days} ${language === "tr" ? "gün" : "d"}`;
}
